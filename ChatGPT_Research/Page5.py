...
class Page5(tk.Frame):
  ...
  def __init__(self, parent, controller):
    ...
    #self.bind_all("<Button-4>", self.on_mousewheel)  # For Linux, scroll up
    #self.bind_all("<Button-5>", self.on_mousewheel)  # For Linux, scroll down
    ...
    self.canvas_frame = self.canvas.create_window((0, 0),
                                                  window=self.container_frame,
                                                  anchor="nw")

    # Configure the container frame to resize correctly
    #self.container_frame.bind("<Configure>",lambda event: self.onFrameConfigure())
    self.container_frame.bind("<Configure>", self.onFrameConfigure)

    # Add mouse wheel scroll functionality
    self.canvas.bind_all("<MouseWheel>",
                         self.on_mousewheel)  # For Windows and MacOS
    self.canvas.bind_all("<Button-4>",
                         self.on_mousewheel)  # For Linux, scroll up
    self.canvas.bind_all("<Button-5>",
                         self.on_mousewheel)  # For Linux, scroll down

    self.video_label = tk.Label(
        self.container_frame)  # Label to display video frames CAMERA
    self.video_label.grid(row=0, column=2, sticky="ne", padx=10,
                          pady=10)  #CAMERA
    self.video_label.grid_remove()

    self.font_style = ("Helvetica", 16, "bold")
    self.title_label = tk.Label(self.container_frame,
                                text="Programming Challenge 1",
                                font=self.font_style)
    self.title_label.grid(row=0, column=0, sticky="w", padx=10, pady=10)

    # Instructions and video button
    self.wLabel = tk.Label(self.container_frame,
                           text='FIRST, Watch this video:',
                           font=self.font_style)
    self.wLabel.grid(row=1, column=0, padx=10, pady=10)
    self.open_link_btn1 = tk.Button(self.container_frame,
                                    text="Variables YouTube Video",
                                    command=self.open_youtube1)
    self.open_link_btn1.grid(row=2, column=0, padx=10, pady=10)

    # Labels and buttons for coding challenge
    self.instruction_label = tk.Label(
        self.container_frame,
        text="Click this button after watching the entire video:",
        font=self.font_style)
    self.instruction_label.grid(row=3, column=0, padx=10, pady=5)
    self.instruction_label.grid_remove()  # Initially hidden

    self.timeLimitLabel = tk.Label(self.container_frame,
                                   text="You will have 15 minutes to code:",
                                   font=self.font_style)
    self.timeLimitLabel.grid(row=4, column=0, padx=10, pady=5)
    self.timeLimitLabel.grid_remove()  # Initially hidden

    self.start_coding_btn = tk.Button(self.container_frame,
                                      text="CLICK HERE TO CODE",
                                      command=self.display_coding_interface)
    self.start_coding_btn.grid(row=6, column=0, padx=10, pady=10)
    self.start_coding_btn.grid_remove()  # Initially hidden
    # Timer setup
    self.timer_label = tk.Label(self.container_frame,
                                text="15:00",
                                font=self.font_style)
    ...
    #Help Image
    self.help_image = Image.open("CH1_Help.PNG")
    self.help_photo = ImageTk.PhotoImage(self.help_image)

    self.container_frame.bind_all("<MouseWheel>",
                                  self.on_mousewheel)  # For Windows and MacOS
  ...
  def on_mousewheel(self, event):  #mouse
    self.canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

  def onFrameConfigure(self, event=None):  #mouse
    """Reset the scroll region to encompass the inner frame"""
    self.canvas.configure(scrollregion=self.canvas.bbox("all"))

  def open_youtube1(self):
    webbrowser.open_new('https://youtu.be/FBeNlZ04NwE')
    #self.open_link_btn1.config(state=tk.DISABLED)
    self.after(569000, self.show_instructions) #569000 ms = 9:29 mins
  ...
  def update_video(self):
    ...
    if self.cap is None:
      ...
      return  # Exit the function if the camera is not initialized
  ...
  def initialize_camera(self):
    ...
    if csv_filename:
      avi_filename = csv_filename.replace(
          '.csv', '.avi')  # RECORD 2 Replace .csv with .avi

    if not self.out:  # Check if the recorder is not already initialized
      self.fourcc = cv2.VideoWriter_fourcc(*'XVID')
    self.out = cv2.VideoWriter(f"CH1{avi_filename}", self.fourcc, 20.0,
                               (320, 240))
  ...
  def display_coding_interface(self):
    if messagebox.askyesno("Confirm",
                           "Are you sure you want to start coding?"):
  ...
  def show_coding_interface(self):
    ...
    obj_label = tk.Label(self.container_frame,
                         text="Objective:",
                         font=font_style)
    ...
    iLabel = tk.Label(
        self.container_frame,
        text=
        '''Write a program that asks the user to input their name (2 pts). 
            First, ask them to enter their birth year (1 pts). 
            Then, ask them to enter the current year (1 pts). 
            Finally, the program should print out: 
            
            "Hello [name]." (2 pts)
            "You were born in [birth year]." (2 pts)
            "The current year is [current year]." (2 pts)'''
            ,
        justify="left",
        wraplength=500,
        font=("Helvetica", 12))
    ...
    noteLabel = tk.Label(
      self.container_frame,
      text = 
      '''Please note that the variables in the '[ ]' 
        must be represented as values from the user.''',
        fg='red', 
        font =('bold', 12))
    noteLabel.grid(row=10, column=0, sticky="w", padx=10, pady=10)

    # Text widget for entering code
    self.text_widget = tk.Text(self.container_frame, height=15, width=50)
    self.text_widget.grid(row=11, column=0, sticky="nsew", padx=10, pady=10)

    submit_btn = tk.Button(self.container_frame,
                           text="Submit",
                           command=self.submit_text)
    submit_btn.grid(row=12, column=0, padx=10, pady=10, sticky="")

    # Create the Help button
    self.help_button = tk.Button(self.container_frame,
                                 text="Help",
                                 command=self.show_help_image)
    self.help_button.grid(row=12, column=1, padx=10, pady=10)
    #self.help_button.grid_remove()
  ...
  def start_timer(self, count):
    ...
    if count > 0:
      self.after(1000, self.start_timer,
                 count - 1)  # Recursive call every second
    else:
      self.timer_label.config(text="Time's up! Submit your code now.",
                              fg="red")
  ...
  def submit_text(self):
    ...
    if messagebox.askyesno("Confirm Submission",
      ...
      self.stop_recording(
      )  # Stop the recording when the user submits their code
      ...
      print(f"Time taken for Challenge 1: {time_str}")
      print(
          f"Challenge 1 completed at: {completion_time.strftime('%Y-%m-%d %H:%M:%S')}"
      )

      # Save the data to a CSV file or any other storage as needed
      self.save_data_to_csv([("Programming Challenge 1", user_input),
                             ("PCH 1 Start Time",
                              self.start_time.strftime('%Y-%m-%d %H:%M:%S')),
                             ("PCH 1 End Time",
                              completion_time.strftime('%Y-%m-%d %H:%M:%S')),
                             ("PCH 1 Time Taken", time_str),
                             ("PCH 1 Keys Pressed", self.key_count)])
      self.stop_keylogger()
      self.controller.show_page(Page6)  # Navigate to the next page

...
class Page9(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.title_label = tk.Label(self.container_frame, text="Programming Challenge 3", font=self.font_style)
        ...
        self.open_link_btn1 = tk.Button(self.container_frame, text="For Loops YouTube Video", command=self.open_youtube1)
        ...
        #Help Image
        self.help_image = Image.open("CH3_Help.PNG")
    ...
    def open_youtube1(self):
        webbrowser.open_new('https://youtu.be/j8kOfwJl7BU')
        #self.open_link_btn1.config(state=tk.DISABLED)
        self.after(528000, self.show_instructions) #528000 ms = 8:48 min
    ...
    def initialize_camera(self):
        ...
        self.out = cv2.VideoWriter(f"CH3{avi_filename}", self.fourcc, 20.0 ,(320, 240)) 
    ...
    def show_coding_interface(self):
        ...
        ch3 = '''Write a program with a for loop that iterates 10 times. (5 pts) 
        Use this for loop to square the numbers from 1 to 10. (5 pts) 
        ...
        iLabel = tk.Label(self.container_frame, text=ch3, justify="left", wraplength=500, font=("Helvetica", 12))
    ...
    def submit_text(self):
        ...
        if messagebox.askyesno("Confirm Submission", "Are you sure you want to submit your code?"):
            ...
            print(f"Time taken for Challenge 3: {time_str}")
            print(f"Challenge 3 completed at: {completion_time.strftime('%Y-%m-%d %H:%M:%S')}")


            # Save the data to a CSV file or any other storage as needed
            self.save_data_to_csv([
            ("Programming Challenge 3", user_input),
            ("PCH 3 Start Time",
             self.start_time.strftime('%Y-%m-%d %H:%M:%S')),
            ("PCH 3 End Time", completion_time.strftime('%Y-%m-%d %H:%M:%S')),
            ("PCH 3 Time Taken", time_str),
            ("PCH 3 Keys Pressed:", self.key_count) 
            ])
            self.stop_keylogger()
            self.controller.show_page(Page10)  # Navigate to the next page 
